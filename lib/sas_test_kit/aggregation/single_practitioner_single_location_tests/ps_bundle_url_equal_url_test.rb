    ...
    module SinglePractitionerSingleLocation
        class BundleUrlEqualUrl < Inferno::Test
            ...
            run do
                ...
                add_message('info', "Query présente dans le Bundle: " + link_query.to_s)
                add_message('info', "Requête présente dans le Bundle: " + link_url)
                add_message('info', "Query présente dans la requête FHIR: " + request_query.to_s)
                add_message('info', "Requête présente dans la requête FHIR: " + request_url)
