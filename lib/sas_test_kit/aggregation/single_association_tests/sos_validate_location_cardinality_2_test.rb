module SasTestKit
    module SingleAssociation
        class ValidateLocationCardinality2 < Inferno::Test
            title "Vérification de la présence de deux ressources Location"
            id :sos_validate_location_cardinality_2
            description %(
                ## Description

                Ce test réalise une vérification de la **présence de deux ressources Location** dans le Bundle de réponse.
            )
            ...
            run do
                ...
                assert(scratch[:locations].length == 2, "Le Bundle doit contenir deux ressources Location, il en possède #{scratch[:locations].length}")
